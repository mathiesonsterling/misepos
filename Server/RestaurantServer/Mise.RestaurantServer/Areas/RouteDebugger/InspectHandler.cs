...
using Mise.RestaurantServer.Areas.RouteDebugger.Models;

namespace Mise.RestaurantServer.Areas.RouteDebugger
{
    /// <summary>
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            if (request.IsInspectRequest())
            {
                ...
                if (response.StatusCode != HttpStatusCode.OK)
                {
                    ...
                    var inspectData = new InspectData(newRequest) {RealHttpStatus = response.StatusCode};
                    response = newRequest.CreateResponse(HttpStatusCode.OK, inspectData);
                }
