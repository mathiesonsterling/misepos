...
namespace Mise.Database.SQLiteDAL
{
	public class SQLitePCLDAL : IRestaurantServerDAL, IClientDAL
	{
        ...
        public SQLitePCLDAL(ILogger logger, IJSONSerializer jsonSerializer, string fileName)
		{
            _logger = logger;
        ...
        Task<bool> IRestaurantServerDAL.UpdateEventStatusesAsync(IEnumerable<IEntityEventBase> events, ItemCacheStatus status)
        {
            throw new NotImplementedException();
        }
        ...
        private bool UpdateEventStatus(IEnumerable<Guid> eventIDs, ItemCacheStatus status)
        {
            
            using (var conn = GetSQLiteConnection())
            {
                var stmt = conn.Prepare(UPDATE_EVENT_STATUS_TEMPLATE);
                foreach (var id in eventIDs)
                {
                    stmt.Reset();
                    stmt.ClearBindings();
                    stmt.Bind("@ID", id.ToString());
                    stmt.Bind("@ItemCacheStatus", status.ToString());
                    //execute it
                    stmt.Step();
                }
            }
        ...
        /*
        private async Task<EntityDataTransportObject> GetEntityByID(Guid id)
        {
            EntityDataTransportObject found = null;
            using (var conn = GetSQLiteConnection())
            {
        ...
        private async Task<IEnumerable<EntityDataTransportObject>> GetEntities(Guid? restaurantID)
        {
            IEnumerable<EntityDataTransportObject> res;
            using (var conn = GetSQLiteConnection())
            {
                await conn.OpenAsync();
        ...
        private static IEnumerable<EntityDataTransportObject> TransformEntityReader(IDataReader reader)
        {
            while (reader.Read())
            {
                yield return new EntityDataTransportObject
                {
                    CreatedDate = DateTime.Parse(reader["CreatedDate"].ToString()),
                    LastUpdatedDate = DateTime.Parse(reader["LastUpdatedDate"].ToString()),
                    Revision = int.Parse(reader["Revision"].ToString()),
...
"CREATE TABLE Entities(ID varchar(100), RestaurantID varchar(100), CreatedDate DATETIME, LastUpdatedDate DATETIME, Revision bigint, JSON Text, ItemCacheStatus varchar(255), Type varchar(1000))";
            ...
            using (var conn = GetSQLiteConnection())
            {
                ...
                using (var trans = conn.BeginTransaction())
                {
                    ...
                    foreach (var ev in events)
                    {
                        ...
                        cmd.Parameters.Add(new SQLiteParameter("@BaseEntityID", ev.BaseEntityID));
    ...
    "UPDATE Entities SET CreatedDate = @CreatedDate, LastUpdatedDate = @LastUpdatedDate, Revision = @Revision, JSON = @JSON, Type=@Type, ItemCacheStatus=@ItemCacheStatus WHERE ID = @ID AND RestaurantID = @RestaurantID";
        ...
        private async Task<bool> UpsertEntities(IEnumerable<EntityDataTransportObject> ents)
        {
            var tasks = new List<Task>();
            using (var conn = GetSQLiteConnection())
            {
